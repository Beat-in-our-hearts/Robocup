...
def run(weights='best.pt',  # model.pt path(s)
        source='../files_dir/temp_path',  # file/dir/URL/glob, 0 for webcam
...
def parse_opt(open_path, save_path, pt_path):
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default=pt_path, help='model.pt path(s)')
    parser.add_argument('--source', type=str, default=open_path, help='file/dir/URL/glob, 0 for webcam')
...
def object_find(open_path, save_path, pt_path):
    """
    物品识别部分
    :param open_path: 打开文件路径
    :param save_path: 保存文件路径
    :param pt_path:   训练模型位置
    :return:
    """
    opt = parse_opt(open_path, save_path, pt_path)
    print(colorstr('detect:') + ', '.join(f'{k}={v}' for k, v in vars(opt).items()))
    check_requirements(exclude=('tensorboard', 'thop'))
    run(**vars(opt))
    print("Object recognition ends...\n")
